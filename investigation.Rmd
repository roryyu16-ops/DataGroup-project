...
set.seed(888)
Deaths_split <- initial_split(CA, prop = 0.80)

train_data <- training(Deaths_split)
test_data  <- testing(Deaths_split)

Deaths_fit <- logistic_reg() %>%
  set_engine("glm") %>%
  fit(rate_group ~ Year + AgeGroup + Sex + Diagnosis,
      data = train_data,
      fimily = "binomial")

Deaths_pred <- predict(Deaths_fit, test_data, type = "prob") %>%
  bind_cols(test_data)
...
Deaths_pred %>%
  mutate(
    rate_group      = if_else(rate_group == 1, "deaths", "alive"),
    rate_group_pred = if_else(.pred_1 > cutoff_prob, "labelled deaths", "labelled alive")
  ) %>%
  count(rate_group_pred, rate_group) %>%
  pivot_wider(names_from = rate_group, values_from = n) %>%
  kable(col.names = c(" ", "alive", "deaths"))
```