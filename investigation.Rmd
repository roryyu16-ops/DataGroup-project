...
Deaths_pred %>%
  mutate(
    rate_group      = if_else(rate_group == 1, "high_risk", "low_risk"),
    rate_group_pred = if_else(.pred_1 > cutoff_prob, "labelled high_risk", "labelled low_risk")
  ) %>%
  count(rate_group_pred, rate_group) %>%
  pivot_wider(names_from = rate_group, values_from = n) %>%
  kable(col.names = c(" ", "low_risk", "high_risk"))
```
